...
describe('Route', () => {
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], TRON, token0)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(TRON)
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, TRON)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(TRON)
  })
})
